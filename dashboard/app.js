...
function editApi(row={}) {
  ...
  const advanced=e('details');advanced.append(e('summary','الإعدادات المتقدمة'),e('p','اترك حقول المفاتيح فارغة للاحتفاظ بها. لإفراغها اكتب {}.','help'),field('headers','Headers (JSON)','','textarea'),field('body','Body (JSON)','','textarea'));
  ...
  const test=button('اختبار الخدمة','play',async()=>{test.disabled=true;testOutput.textContent='جارٍ اختبار الطلب...';try{const v=values();v.timeoutMs=Number(v.timeoutMs);for(const key of ['headers','body']){if(!v[key].trim())delete v[key];else {try{v[key]=JSON.parse(v[key])}catch{throw Error('تحقق من صيغة '+key)}}}const result=await request('apis/test','POST',{...v,testQuery:document.getElementById('field_testQuery').value,testPath:document.getElementById('field_testPath').value});testOutput.textContent='نجح الطلب ✓\n'+JSON.stringify(result,null,2);}catch(error){testOutput.textContent='فشل الطلب ✕\n'+error.message;}finally{test.disabled=false;}});
