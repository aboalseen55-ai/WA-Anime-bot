...
function editApi(row={}) {
  ...
  openEditor(row._id?'تعديل الخدمة':'إضافة خدمة',[grid,advanced],async()=>{const v=values();v.timeoutMs=Number(v.timeoutMs);for(const key of ['headers','body']){if(!v[key].trim())delete v[key];else {try{v[key]=JSON.parse(v[key])}catch{throw Error('تحقق من صيغة '+key)}}}await request('apis'+(row._id?'/'+row._id:''),row._id?'PUT':'POST',v);await saved()});
}
